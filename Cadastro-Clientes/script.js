document.addEventListener('DOMContentLoaded', function() {
    ...
    function viewDocuments(clientId) {
      ...
      if (client.documentos && client.documentos.length > 0) {
        client.documentos.forEach((doc, index) => {
          const docItem = document.createElement('div');
          docItem.className = 'document-item';
          docItem.innerHTML = `
            <p><strong>Documento ${index + 1}:</strong> ${doc.name}</p>
            <p>Tipo: ${doc.type} | Tamanho: ${formatFileSize(doc.size)}</p>
            <a href="data:${doc.type};base64,${doc.data}" download="${doc.name}">Baixar Documento</a>
          `;
          documentsList.appendChild(docItem);
        });
      } else {
        documentsList.innerHTML = '<p>Nenhum documento anexado para este cliente.</p>';
      }
